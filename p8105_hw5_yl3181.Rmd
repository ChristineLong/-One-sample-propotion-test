...
homi = raw_homi %>% 
  ...
  mutate(prop_homi = unsol_homi / total_homi)

# One sample propotion test for Baltimore, MD
prop.test(homi[homi$city_state == "Baltimore_MD",]$prop_homi, homi[homi$city_state == "Baltimore_MD",]$total_homi) %>% 
  broom::tidy() %>% 
  select(estimate, starts_with("conf."))

# One sample propotion test for all cities
test_homi = purrr::map2(homi$prop_homi, homi$total_homi, prop.test)
